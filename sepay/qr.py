...
# (img.vietqr.io/qr_only|print vẫn nhúng logo chữ V). Payload EMV mã hoá y hệt nên
# app ngân hàng đọc như cũ.
...
def qr_image_url(bank_name: str, account_number: str, account_name: str, amount: int, note: str) -> str:
    ...
    img.vietqr.io/qr_only|print vẫn baked logo VietQR). Payload EMV mã hoá GIỐNG HỆT:
    bank BIN + số TK + amount + nội dung (des) → app ngân hàng đọc y như cũ, tự điền
    số tiền & nội dung. UI tự vẽ thương hiệu + STK/nội dung nên dùng showinfo=false để
    mã QR to & sạch nhất (không baked text). `account_name` chỉ dùng cho showinfo
    (đang tắt) — không nằm trong payload EMV.
    """

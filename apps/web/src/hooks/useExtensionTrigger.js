    ...
    return new Promise((resolve) => {
        ...
        const onMessage = (ev) => {
            if (ev.source !== window)
                return;
            const data = ev.data;
            if (data?.source !== "autogpt-extension")
                return;
            if (data.type !== "pong" && data.type !== "bridge-ready")
                return;
            if (resolved)
                return;
            resolved = true;
        ...
        window.postMessage({ source: "autogpt-dashboard", type: "ping" }, window.origin);
        setTimeout(() => {
            if (resolved)
                return;
            resolved = true;
...
function bridgeMissingMessage() {
    return ("Extension chưa được dashboard nhận diện.\n\n" +
 ...
 */
export async function triggerExtensionRun() {
    const version = await probeBridge();
    console.log(`[autogpt-dashboard] triggerExtensionRun — bridge v${version ?? "MISSING"}`);
    if (version === null) {
        window.alert(bridgeMissingMessage());
        return false;
    }
    window.postMessage({ source: "autogpt-dashboard", type: "run-pending" }, window.origin);
    return true;
}
/**

...
### 2026-05-17 — UI fix: bỏ chấm thoi đen + AutoGPT brand click → home
- **Loại**: UI / navigation
- **Mô tả**: Bỏ chấm thoi (`width:8 height:8 rotate(45deg)`) cạnh trái chữ "AutoGPT" trong sidebar (và cả trang Login để đồng bộ). Brand mark "AutoGPT / Admin" trong sidebar bọc trong `<Link to="/workspaces">` để click về trang chủ. Cursor đổi sang pointer, không gạch chân (textDecoration none, color inherit).
- **Tại sao**: User feedback 2026-05-17 — "autogpt bỏ cái dấu chấm đen bên trái đi. ấn vào nó sẽ là trang chủ."
- **File đã đổi**: [Layout.tsx](../../apps/web/src/components/Layout.tsx), [Login.tsx](../../apps/web/src/pages/Login.tsx).

### 2026-05-17 — UI redesign: design system mới (IBM Plex + cream/beige tokens)
- **Loại**: UI / design system
- **Mô tả**: Trang Login redesign theo mockup — brand mark hình thoi + chữ "AutoGPT / Admin" mono uppercase, form đặt trong `.surface-card` với padding 32px, error message dùng style `danger-bg + border` thay panel rose, nút submit full-width dùng `.btn .btn-primary`. Tất cả copy text giữ nguyên qua i18n.
- **Tại sao**: Đồng bộ với toàn bộ dashboard sau khi áp dụng design system mới (tokens + component classes trong [index.css](../../apps/web/src/index.css)).
- **Logic không đổi**: hook `useAuth.login`, error handling `ApiError`, redirect via `location.state.from` giữ nguyên.
- **File đã đổi**: [Login.tsx](../../apps/web/src/pages/Login.tsx), [index.css](../../apps/web/src/index.css), [index.html](../../apps/web/index.html) (preload IBM Plex Sans/Mono).
